...
def case_refund():
    ...
    refund_display = (
        refund_bill.loc[refund_bill["state"] == 2]
        .loc[
            :,
            [
                "case_code",
    ...
    refund_display = transform_case_code(refund_display)
    refund_display['refund_amount'] = refund_display['refund_amount'].astype(float)

    case_code_map = pd.read_sql("select * from case_code_map", sqliteEngine)
    
    refund_display = pd.merge(refund_display, case_code_map, on='case_code_m', how='left')
    ...
    st.data_editor(
        ...
        column_config={
            ...
            "create_time": st.column_config.DateColumn("申请日期", disabled=True),
            "refund_date": st.column_config.DateColumn("会计确认日期", disabled=True),
